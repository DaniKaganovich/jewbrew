...
void ProcessBeerCloneBrau()
{
	...
	case 2: // Температура Больше необходиомй
		ProcessPIDTemp((int)tempP[KlTek] * 10, temps[TEMP_TERMOSTAT]);
		UstPower = (long)Power*KtT / 1000;
		// Мощность ограничена мощностью варки зерна
		if (UstPower > PowerVarkaZerno) UstPower = PowerVarkaZerno;
		...
		//  if (temps[TEMP_DEFL]<TempDeflBegDistil && ds1820_devices>1 || (TempDeflBegDistil<0 &&  temps[TEMP_DEFL]<-TempDeflBegDistil) && tempP[KlTek]>=110 )  break;
	...
	case 3:
		if (tempP[KlTek] > 100)
		{
			...
			if (temps[TEMP_DEFL] < TempDeflBegDistil && ds1820_devices>1 || (TempDeflBegDistil < 0 && temps[TEMP_DEFL] < -TempDeflBegDistil) && tempP[KlTek] >= 110)  time2 = (int)timeP[KlTek] * 60;

		}
		else
		{
			if (temps[TEMP_TERMOSTAT] < (int)tempP[KlTek] * 10)
			{
				ProcessPIDTemp((int)tempP[KlTek] * 10, temps[TEMP_TERMOSTAT]);
				UstPower = (long)Power*KtT / 1000;
				if (UstPower > PowerVarkaZerno) UstPower = PowerVarkaZerno;
...
void ProcessDistilDefl()
{
	...
	case 2: // Ожидание, пока прогреется термометр в дефлегматоре
		...
		if (temps[TEMP_DEFL] < TempDeflBegDistil && ds1820_devices>1 || (TempDeflBegDistil < 0 && temps[TEMP_KUB] < -TempDeflBegDistil)) break;
...
void ProcessNDRF()
{
	...
	if (temps[TEMP_KUB] >= tEndRect && StateMachine < 8 && IspReg != 118)
	{
		StateMachine = 8;
		SecTempPrev = Seconds;// Запомним время, когда закончилась ректификация
		if (BeepStateProcess) my_beep(BEEP_LONG);
	...
	case 2: // Разгон
		...
		if (temps[TEMP_KUB] < tEndRectRazgon  && tEndRectRazgon>0) break;
		if (temps[TEMP_RK20] < -tEndRectRazgon && tEndRectRazgon <= 0) break;
	...
	case 3:// Стабилизация температуры (строго по времени)
		...
		if (temps[TEMP_RK20] > 0) // Если температура больше 70 градусов (вдруг куб прогрелся, а колонна еще нет, нелогично, конечно, но на всякий случай учтем)
		{
...
void ProcessRazvarZerno()
{
	...
	case 1: //Нагрев до температуры 50 градусов
	  ...
	  //    if (TempZSPSld<55) TempZSPSld=55;
		...
		//  
		//  
		if (temps[TEMP_RAZVAR] < (int)TempZSP * 10)
		{
			ProcessPIDTemp((int)TempZSP * 10, temps[TEMP_RAZVAR]);
			UstPower = (long)Power*KtT / 1000;
			// Мощность ограничена мощностью варки зерна
			if (UstPower > PowerVarkaZerno) UstPower = PowerVarkaZerno;
			break;
	...
	case 2: //Поддержание температуры 50 градусов, пока не произойдет ручной переход к следующему этапу.
	  ...
	  // Пищим, чтобы привлечь внимание
		if (BeepStateProcess) my_beep(BEEP_LONG);
		ProcessPIDTemp(500, temps[TEMP_RAZVAR]);
		UstPower = (long)Power*KtT / 1000;
		// Мощность ограничена мощностью варки зерна
		if (UstPower > PowerVarkaZerno) UstPower = PowerVarkaZerno;
		break;
	...
	case 3: //Нагрев до температуры 64 градуса
		if (temps[TEMP_RAZVAR] < (int)TempZSPSld * 10)
		{
			...
			ProcessPIDTemp((int)TempZSPSld * 10, temps[TEMP_RAZVAR]);
			UstPower = (long)Power*KtT / 1000;
			// Мощность ограничена мощностью варки зерна
			if (UstPower > PowerVarkaZerno) UstPower = PowerVarkaZerno;
			break;
	...
	case 12: // Остужение до 355 градусов
		...
		if (temps[TEMP_RAZVAR] <= (int)TempHLDZatorBrog1 * 10)
		{
	...
	case 14: // Поддержка брожения температура превышена
	  // Поперла пена
		...
		if (temps[TEMP_RAZVAR] > (int)TempHLDZatorBrog1 * 10 - 5)
		{
	...
	case 6: // Отбор СР
	  ...
	  // Работа Клапана отбора голов зависит от работы царги пастеризации
		...
		if (lastStableT < 500) {
			...
			lastStableT = (lastStableT+float(temps[TEMP_RK20])/tStabTimeConstant) / tStabAverageDivisor;
		...
		else // if (temps[TEMP_RK20]>=tStabSR+tDeltaRect)
		{
		...
		}	// if (temps[TEMP_RK20]>=tStabSR+tDeltaRect)
