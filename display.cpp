...
{
	if (TempDeflBegDistil > 0) sprintf_P(lcd_buffer, PSTR("tD=%3i/%3i %4iW"), temps[TEMP_DEFL], TempDeflBegDistil, Power);
	else  sprintf_P(lcd_buffer, PSTR("tK=%3i/%3i %4iW"), temps[TEMP_KUB], -TempDeflBegDistil, Power);
...
void formTSAErr()
{
	...
	sprintf_P(lcd_buffer, PSTR("tT=%3i/%3i"), temps[TEMP_TSA], MAX_TEMP_TSA);
}
...
void DisplayRectif()
{
	if (DispPage == 0)
	{
		...
		case 1: //Не запущено
			...
			//sprintf_P(lcd_buffer,PSTR("tK=%3i P=%4i"),temps[TEMP_KUB],Power); 
			break;
		case 2: //Разгон
			sprintf_P(lcd_buffer, PSTR("%02u:%02u:%02u R RZG"), hour, minute, second);
			//sprintf_P(lcd_buffer,PSTR("%u%u%u%u  %u%u%u%u"),PINB.4,PINB.3,PINB.2,PINA.7, PINA.6,PINA.5,PINA.4,PINA.3);                     
			my_lcdprint(lcd_buffer); //вывод содержимого буфера на LCD
			if (tEndRectRazgon > 0) sprintf_P(lcd_buffer, PSTR("tK=%3i/%3i %4iW"), temps[TEMP_KUB], tEndRectRazgon, UstPower);
			else sprintf_P(lcd_buffer, PSTR("tN=%3i/%3i %4iW"), temps[TEMP_RK20], -tEndRectRazgon, UstPower);
			break;
		case 3: //Стабилицация колонны
			sprintf_P(lcd_buffer, PSTR("%02u:%02u:%02u R NSB"), hour, minute, second);
			//sprintf_P(lcd_buffer,PSTR("%u%u%u%u  %u%u%u%u"),PINB.4,PINB.3,PINB.2,PINA.7, PINA.6,PINA.5,PINA.4,PINA.3);                     
			my_lcdprint(lcd_buffer); //вывод содержимого буфера на LCD
			sprintf_P(lcd_buffer, PSTR("tN=%3i(%3i)%4iW"), temps[TEMP_RK20], (IspReg == 111 ? (int)(SecOstatok) : (int)(Seconds - SecTempPrev)), PowerRect);
			break;
		case 4: //Отбор голов
			if (IspReg != 118)
			{
				...
				if (UrovenProvodimostSR == 0)
					sprintf_P(lcd_buffer, PSTR("tK=%3i/%3i %4iW"), temps[TEMP_KUB], tEndRectOtbGlv, PowerRect);
				else
					if (UrovenProvodimostSR > 0)
					{
			...
			else
			{
				...
				if (V2 >= 0) sprintf_P(lcd_buffer, PSTR("tK=%3i/%3i %4iW"), temps[TEMP_KUB], V2, UstPower);
		...
		case 5: //Стоп, ожидание возврата температуры
			...
			//sprintf_P(lcd_buffer,PSTR("%u%u%u%u  %u%u%u%u"),PINB.4,PINB.3,PINB.2,PINA.7, PINA.6,PINA.5,PINA.4,PINA.3);                     
			my_lcdprint(lcd_buffer); //вывод содержимого буфера на LCD
			sprintf_P(lcd_buffer, PSTR("tN=%3i/%3i %2i%%"), temps[TEMP_RK20], tStabSR, ProcChimSR);
			break;
		case 6: //Ректификация
			sprintf_P(lcd_buffer, PSTR("%02u:%02u:%02u RSt %2i%%"), hour, minute, second, ProcChimSR);
			//sprintf_P(lcd_buffer,PSTR("%u%u%u%u  %u%u%u%u"),PINB.4,PINB.3,PINB.2,PINA.7, PINA.6,PINA.5,PINA.4,PINA.3);                     
			my_lcdprint(lcd_buffer); //вывод содержимого буфера на LCD
			sprintf_P(lcd_buffer, PSTR("tN=%3i/%3i/d%2i"), temps[TEMP_RK20], tStabSR + tDeltaRect, tDeltaRect);
			break;
		case 7: //Отбор хвостов
			sprintf_P(lcd_buffer, PSTR("%02u:%02u:%02u R Hvost"), hour, minute, second);
			//sprintf_P(lcd_buffer,PSTR("%u%u%u%u  %u%u%u%u"),PINB.4,PINB.3,PINB.2,PINA.7, PINA.6,PINA.5,PINA.4,PINA.3);                     
			my_lcdprint(lcd_buffer); //вывод содержимого буфера на LCD
			sprintf_P(lcd_buffer, PSTR("tK=%3i/%3i"), temps[TEMP_KUB], IspReg != 118 ? tEndRect : TekTemp);
			break;
		...
		case 100: //Окончание
			...
			//sprintf_P(lcd_buffer,PSTR("%u%u%u%u  %u%u%u%u"),PINB.4,PINB.3,PINB.2,PINA.7, PINA.6,PINA.5,PINA.4,PINA.3);                     
			my_lcdprint(lcd_buffer); //вывод содержимого буфера на LCD
			sprintf_P(lcd_buffer, PSTR("tK=%3i/%3i End"), temps[TEMP_KUB], tEndRect);
			break;
	...
	{
		sprintf_P(lcd_buffer, PSTR("D2 T=%3i,%3i,%3i"), temps[TEMP_KUB], temps[TEMP_RK20], temps[TEMP_TSA]);
...
void DisplayDistDefl()
{
	if (DispPage == 0)
	{
		...
		case 2: //Ожидание закипания (прогреется дефлегматор)
			...
			my_lcdprint(lcd_buffer); //вывод содержимого буфера на LCD
			sprintf_P(lcd_buffer, PSTR("tD=%3i/%3i %4iW"), temps[TEMP_DEFL], TempDeflBegDistil, UstPower);
			break;
		case 3: //Ожидание закипания (прогреется куб дефлегматор)
			sprintf_P(lcd_buffer, PSTR("%02u:%02u:%02u DD Rzg"), hour, minute, second);
			my_lcdprint(lcd_buffer); //вывод содержимого буфера на LCD
			sprintf_P(lcd_buffer, PSTR("tK=%3i/%3i %4iW"), temps[TEMP_KUB], TempDeflBegDistil, UstPower);
			break;
		case 4: //Работа без дефлегматора
			sprintf_P(lcd_buffer, PSTR("%02u:%02u:%02u DD BezD"), hour, minute, second);
			my_lcdprint(lcd_buffer); //вывод содержимого буфера на LCD
			sprintf_P(lcd_buffer, PSTR("tD=%3i/%3i %4iW"), temps[TEMP_DEFL], TempDefl, UstPower);
			break;
		case 5: //Работа с 50% дефлегмацией
			sprintf_P(lcd_buffer, PSTR("%02u:%02u:%02u DD 50%%"), hour, minute, second);
			my_lcdprint(lcd_buffer); //вывод содержимого буфера на LCD
			sprintf_P(lcd_buffer, PSTR("tD=%3i/%3i %4iW"), temps[TEMP_DEFL], TempDefl - DeltaDefl, UstPower);
			break;
		...
		case 7: //Ожидание для охлаждения
			...
			my_lcdprint(lcd_buffer); //вывод содержимого буфера на LCD
			sprintf_P(lcd_buffer, PSTR("tK=%3i Wait=%4i"), temps[TEMP_KUB], SecondsEnd);
			break;
		case 100: //Окончание
			sprintf_P(lcd_buffer, PSTR("%02u:%02u:%02u DD end"), hour, minute, second);
			my_lcdprint(lcd_buffer); //вывод содержимого буфера на LCD
			sprintf_P(lcd_buffer, PSTR("tK=%3i end"), temps[TEMP_KUB]);
			break;
		}
	}
	else
	{
		sprintf_P(lcd_buffer, PSTR("D2 T=%3i,%3i,%3i"), temps[TEMP_KUB], temps[TEMP_DEFL], temps[TEMP_TSA]);
		my_lcdprint(lcd_buffer);
...
void DisplaySimpleGLV()
 ...
 sprintf_P(lcd_buffer,PSTR("%02u:%02u:%02u sGl Rzg"),hour,minute,second);
 my_lcdprint(lcd_buffer); //вывод содержимого буфера на LCD
 sprintf_P(lcd_buffer,PSTR("tK=%3i/%3i %4iW"),temps[TEMP_KUB],TempDeflBegDistil,Power);
 break;
 case 4: //Отбор голов
 sprintf_P(lcd_buffer,PSTR("%02u:%02u:%02u sGl Otb"),hour,minute,second);
 my_lcdprint(lcd_buffer); //вывод содержимого буфера на LCD
 sprintf_P(lcd_buffer,PSTR("tK=%3i/%3i %4iW"),temps[TEMP_KUB],Temp3P,PowerGlvDistil);
 break;
 ...
 {
 sprintf_P(lcd_buffer,PSTR("D2 T=%3u,%3u,%3u"),temps[TEMP_KUB],temps[TEMP_DEFL],temps[TEMP_TSA]);
 my_lcdprint(lcd_buffer);
...
void DisplaySimpleDistill()
{
	if (DispPage == 0)
	{
		...
		case 3: //Дистилляция
			my_lcdprint(lcd_buffer); //вывод содержимого буфера на LCD
			sprintf_P(lcd_buffer, PSTR("tK=%3i/%3i %4iW"), temps[TEMP_KUB], V2, UstPower);
			break;
...
void DisplayFracionDistill()
{
	if (DispPage == 0)
	{
		...
		case 4: //Ожидание 60 секунд для охлаждения
			my_lcdprint(lcd_buffer); //вывод содержимого буфера на LCD
			sprintf_P(lcd_buffer, PSTR("tK=%3i Wait=%4i"), temps[TEMP_KUB], time3);
			break;
		case 100: //Окончание
			my_lcdprint(lcd_buffer); //вывод содержимого буфера на LCD
			sprintf_P(lcd_buffer, PSTR("tK=%3i end"), temps[TEMP_KUB]);
...
void DisplayRazvar()
{
	if (DispPage == 0)
	{
		...
		case 0: //Не запущено
		  //            case 1: //Нагрев до температуры 50 градусов
		  //              sprintf_P(lcd_buffer,PSTR("t=%3i P=%4i"),temps[TEMP_RAZVAR],UstPower); 
		  //              break;
		case 1: //Нагрев до температуры 50 градусов
			sprintf_P(lcd_buffer, PSTR("t=%3i/%3i %4iW"), temps[TEMP_RAZVAR], (int)TempZSP * 10, UstPower);
			break;
		case 2: //Поддержание температуры 50 градусов, пока не произойдет ручной переход к следующему этапу.
			sprintf_P(lcd_buffer, PSTR("t=%3i ZASYP & Dn"), temps[TEMP_RAZVAR]);
			break;
		case 3: //Нагрев до температуры 64 градуса
			sprintf_P(lcd_buffer, PSTR("t=%3i/%3i %4iW"), temps[TEMP_RAZVAR], (int)TempZSPSld * 10, UstPower);
			break;
		case 4: //Ожидание 15 минут, поддержка температуры
			sprintf_P(lcd_buffer, PSTR("t=%3i %4iW %4i"), temps[TEMP_RAZVAR], UstPower, time2);
			break;
		case 5: //Нагрев до кипения
			if (ds1820_devices < 2)
				sprintf_P(lcd_buffer, PSTR("tK=%3i/%3i %4iW"), temps[TEMP_RAZVAR], TempKipenZator, UstPower);
			else
				sprintf_P(lcd_buffer, PSTR("tD=%3i/%3i %4iW"), temps[TEMP_DEFL], TempKipenZator, UstPower);
			break;
		case 6: //Варка
			sprintf_P(lcd_buffer, PSTR("t=%3i %4iW %4i"), temps[TEMP_RAZVAR], PowerRazvZerno, time2);
			break;
		case 7: //Охлаждение до температыры осахаривания
			sprintf_P(lcd_buffer, PSTR("tK=%3i/%3i HLD-1"), temps[TEMP_RAZVAR], (int)TempZSPSld * 10);
			break;
		case 8: //Поддержание температуры 50 градусов, пока не произойдет ручной переход к следующему этапу.
			sprintf_P(lcd_buffer, PSTR("t=%3i SOLOD & Dn"), temps[TEMP_RAZVAR]);
			break;
		case 9: //Мешаем 10 минут
			sprintf_P(lcd_buffer, PSTR("tK=%3i MIX %4i"), temps[TEMP_RAZVAR], time2);
			break;
		case 10: //Осахаривание
			sprintf_P(lcd_buffer, PSTR("tK=%3i OSH %4i"), temps[TEMP_RAZVAR], time2);
			break;
		case 11: //Охлаждение до температуры первичного внесения дрожжей осахаривания
			sprintf_P(lcd_buffer, PSTR("tK=%3i/400 HLD-2"), temps[TEMP_RAZVAR]);
			break;
		case 12: //Охлаждение до температыры осахаривания
			sprintf_P(lcd_buffer, PSTR("tK=%3i/%3i HLD-3"), temps[TEMP_RAZVAR], (int)TempHLDZatorBrog1 * 10);
			break;
		case 13: //Поддержка брожения, ничего не делаем, только мешаем периодически
			sprintf_P(lcd_buffer, PSTR("tK=%3i/%3i BROG"), temps[TEMP_RAZVAR], (int)TempHLDZatorBrog1 * 10);
			break;
		case 14: //Поддержка брожения - охлаждение
			sprintf_P(lcd_buffer, PSTR("tK=%3i/%3i BR-H"), temps[TEMP_RAZVAR], (int)TempHLDZatorBrog1 * 10 - 5);
			break;
		case 100: //Окончание
			sprintf_P(lcd_buffer, PSTR("t=%3i end"), temps[TEMP_RAZVAR]);
			break;
		}
	}
	else
...
void DisplayNDRF()
 ...
 //sprintf_P(lcd_buffer,PSTR("%u%u%u%u  %u%u%u%u"),PINB.4,PINB.3,PINB.2,PINA.7, PINA.6,PINA.5,PINA.4,PINA.3);
 my_lcdprint(lcd_buffer); //вывод содержимого буфера на LCD
 sprintf_P(lcd_buffer,PSTR("t=%3i(%4i)%4iW"),temps[TEMP_RK20],(int)(SecOstatok),PowerRect);
 break;
 case 4: //Отбор голов
 ...
 //sprintf_P(lcd_buffer,PSTR("%u%u%u%u  %u%u%u%u"),PINB.4,PINB.3,PINB.2,PINA.7, PINA.6,PINA.5,PINA.4,PINA.3);
 my_lcdprint(lcd_buffer); //вывод содержимого буфера на LCD
 sprintf_P(lcd_buffer,PSTR("tK=%3i/%3i"),temps[TEMP_KUB],tEndRect);
 break;
 case 8: //Отбор ожидание 3 минут
...
void DisplayNBK()
{
	if (DispPage == 0)
	{
		...
		case 0: //Не запущено
			...
			//sprintf_P(lcd_buffer,PSTR("%u%u%u%u  %u%u%u%u"),PINB.4,PINB.3,PINB.2,PINA.7, PINA.6,PINA.5,PINA.4,PINA.3);                     
			my_lcdprint(lcd_buffer); //вывод содержимого буфера на LCD
			sprintf_P(lcd_buffer, PSTR("tK=%3i,%3imm"), temps[TEMP_KUB], U_MPX5010);
			break;
		...
		case 5: //Превышение температуры вверху
			...
			//sprintf_P(lcd_buffer,PSTR("%u%u%u%u  %u%u%u%u"),PINB.4,PINB.3,PINB.2,PINA.7, PINA.6,PINA.5,PINA.4,PINA.3);                     
			my_lcdprint(lcd_buffer); //вывод содержимого буфера на LCD
			sprintf_P(lcd_buffer, PSTR("tD=%3i/970,s%4i"), temps[TEMP_DEFL], time2);
			break;
		case 100: //Окончание
...
void DisplayTestKLP()
{
	...
	else
	{
		sprintf_P(lcd_buffer, PSTR("D2 T=%3i,%3i,%3i"), temps[TEMP_KUB], temps[TEMP_DEFL], temps[TEMP_TSA]);
		my_lcdprint(lcd_buffer);
		sprintf_P(lcd_buffer, PSTR("%3u,%3u Gas=%4u"), temps[3], temps[4], U_GAS);
	}
...
void DisplayExtContol()
{
	if (DispPage == 0)
	{
		...
		//sprintf_P(lcd_buffer,PSTR("%u%u%u%u  %u%u%u%u"),PINB.4,PINB.3,PINB.2,PINA.7, PINA.6,PINA.5,PINA.4,PINA.3);                     
		my_lcdprint(lcd_buffer); //вывод содержимого буфера на LCD
		sprintf_P(lcd_buffer, PSTR("T=%3u,%3u %4iW"), temps[TEMP_KUB], temps[TEMP_DEFL], UstPower);
	}
	else
	{
		sprintf_P(lcd_buffer, PSTR("D2 T=%3i,%3i,%3i"), temps[TEMP_KUB], temps[TEMP_DEFL], temps[TEMP_TSA]);
...
void DisplayBeer()
{
	if (DispPage == 0)
	{
		...
		my_lcdprint(lcd_buffer); //вывод содержимого буфера на LCD
		if (StateMachine == 2)   sprintf_P(lcd_buffer, PSTR("T=%3i/%3i P%2i/%2i"), temps[TEMP_TERMOSTAT], (int)tempP[KlTek] * 10, (int)KlTek, (int)CntPause);
		if (StateMachine == 3)
		{
			if (timeP[KlTek] != 0)
...
void DisplayData()
{
	...
	case 0:
		...
		if (DispPage < 2) // Первые две страницы отображаются по-разному, остальные всегда одни и те же
		{
			...
			case 102: // Термостат         
				if (DispPage == 0)
				{
					...
					my_lcdprint(lcd_buffer); //вывод содержимого буфера на LCD
					sprintf_P(lcd_buffer, PSTR("t=%3i/%3i(%2i) %1i"), temps[TEMP_TERMOSTAT], TempTerm, Delta, StateMachine);
				}
				else
				{
					sprintf_P(lcd_buffer, PSTR("D2 T=%3i,%3i,%3i"), temps[TEMP_KUB], temps[TEMP_DEFL], temps[TEMP_TSA]);
					my_lcdprint(lcd_buffer);
					sprintf_P(lcd_buffer, PSTR("%3u,%3u P=%3imm"), temps[3], temps[4], U_MPX5010);
				}
				break;
			case 115: // Таймер
				if (DispPage == 0)
				...
				else
				{
					sprintf_P(lcd_buffer, PSTR("D2 T=%3i,%3i,%3i"), temps[TEMP_KUB], temps[TEMP_DEFL], temps[TEMP_TSA]);
					my_lcdprint(lcd_buffer);
					sprintf_P(lcd_buffer, PSTR("%3u,%3u %4iW"), temps[3], temps[4], PowerMinute);
				}
				break;
		...
		if (DispPage == 3)
		{
			sprintf_P(lcd_buffer, PSTR("D4 T=%3i,%3i,%3i"), temps[TEMP_KUB], temps[TEMP_DEFL], temps[TEMP_TSA]);
			my_lcdprint(lcd_buffer);
			sprintf_P(lcd_buffer, PSTR("%3i,%3i P=%3imm"), temps[3], temps[4], U_MPX5010);
		}
		...
		if (DispPage == 9)
		{
			sprintf_P(lcd_buffer, PSTR("D10 MaxT=%3i,%3i"), Maxtemps[0], Maxtemps[1]);
			my_lcdprint(lcd_buffer);
			sprintf_P(lcd_buffer, PSTR("%3i,%3i,%3i"), Maxtemps[2], Maxtemps[3], Maxtemps[4]);
		}
	...
	case 111:// НДРФ
		...
		break;
	case 112:// НБК
		sprintf_P(lcd_buffer, PSTR("IsR = NBK"));
		my_lcdprint(lcd_buffer); //вывод содержимого буфера на LCD
		sprintf_P(lcd_buffer, PSTR("                "));
		lcd.setCursor(0, 1);
		my_lcdprint(lcd_buffer); //вывод содержимого буфера на LCD
	...
	case 268:
		...
		break;
	case 300:
		sprintf_P(lcd_buffer, PSTR("Temp%1i=%3u"), nPopr, temps[nPopr]);
		my_lcdprint(lcd_buffer); //вывод содержимого буфера на LCD
		sprintf_P(lcd_buffer, PSTR("Popr=%3i T=%3u"), ds1820_popr[nPopr], (temps[nPopr] + ds1820_popr[nPopr]));
		lcd.setCursor(0, 1);
		my_lcdprint(lcd_buffer); //вывод содержимого буфера на LCD
		break;
	case 301:
