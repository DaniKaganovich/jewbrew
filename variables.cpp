...
unsigned int timeChimRectOtbGlv=1000;  // Шим отбора голов (в полупериодах сетевого напряжения в данном случае это около 20 секунд)
char ProcChimOtbGlv=12;  // Процент ШИМ отбора голов (в данном случае это 5 процентов)
...
char tDeltaRect=2;  // Дельта ректификации (в десятых градуса Цельсия) в данном случае - это 10 десятых, то есть 1 градус Цельсия
...
char ProcChimOtbCP=0;  // Процент ШИМ отбора голов при работе с царгой пастеризации положительные значения-отбор относительно отбора голов, отрицательные - абсолютное значение
